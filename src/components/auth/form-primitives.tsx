...
>(({ className, ...props }, ref) => {
  ...
  return (
    <Label
      ref={ref}
      className={cn(
        "text-[13px] font-semibold uppercase tracking-wider text-text-muted transition-colors",
        error && "text-error",
        className,
      )}
      htmlFor={formItemId}
...
>(({ className, children, ...props }, ref) => {
  ...
  return (
    <p
      ...
      id={formMessageId}
      className={cn(
        "text-xs font-medium text-error animate-in slide-in-from-top-1",
        className,
      )}
      {...props}
    >
...
// Optimized isolated field that doesn't re-render parent
